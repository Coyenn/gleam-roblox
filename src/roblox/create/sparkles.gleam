...
import roblox/types.{type Color3, type Instance, type SecurityCapabilities, type Sparkles}
...
@target(luau)
pub fn node(properties: List(Property(Sparkles)), children: List(Node)) -> Node {
...
pub fn local_transparency_modifier(value: Float) -> Property(Sparkles) {
  Property(fn(instance) { sparkles.set_local_transparency_modifier(instance, value) })
}
...
pub fn javascript_type_anchor(_: Instance, _: SecurityCapabilities, _: Color3, _: Sparkles) -> Nil {
  Nil
}