...
import api/audio_equalizer
import api/types.{type AudioEqualizer, type Instance, type NumberRange, type SecurityCapabilities}
...
@target(luau)
pub fn node(properties: List(Property(AudioEqualizer)), children: List(Node)) -> Node {
...
pub fn javascript_type_anchor(_: Instance, _: SecurityCapabilities, _: NumberRange, _: AudioEqualizer) -> Nil {
  Nil
}