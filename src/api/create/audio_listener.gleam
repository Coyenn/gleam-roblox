...
import api/audio_listener
import api/types.{type AudioListener, type Instance, type SecurityCapabilities}
...
@target(luau)
pub fn node(properties: List(Property(AudioListener)), children: List(Node)) -> Node {
...
pub fn acoustic_simulation_enabled(value: Bool) -> Property(AudioListener) {
  Property(fn(instance) { audio_listener.set_acoustic_simulation_enabled(instance, value) })
}
...
pub fn audio_interaction_group(value: String) -> Property(AudioListener) {
  Property(fn(instance) { audio_listener.set_audio_interaction_group(instance, value) })
}
...
pub fn javascript_type_anchor(_: Instance, _: SecurityCapabilities, _: AudioListener) -> Nil {
  Nil
}