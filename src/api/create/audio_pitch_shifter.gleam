...
import api/audio_pitch_shifter
import api/types.{type AudioPitchShifter, type AudioWindowSize, type Instance, type SecurityCapabilities}
...
@target(luau)
pub fn node(properties: List(Property(AudioPitchShifter)), children: List(Node)) -> Node {
...
@target(luau)
pub fn capabilities(value: SecurityCapabilities) -> Property(AudioPitchShifter) {
  Property(fn(instance) { audio_pitch_shifter.set_capabilities(instance, value) })
}
...
pub fn javascript_type_anchor(_: Instance, _: SecurityCapabilities, _: AudioWindowSize, _: AudioPitchShifter) -> Nil {
  Nil
}