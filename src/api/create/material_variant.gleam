...
import api/material_variant
import api/types.{type AlphaMode, type Color3, type Instance, type MaterialPattern, type MaterialVariant, type PhysicalProperties, type SecurityCapabilities}
...
@target(luau)
pub fn node(properties: List(Property(MaterialVariant)), children: List(Node)) -> Node {
...
@target(luau)
pub fn custom_physical_properties(value: PhysicalProperties) -> Property(MaterialVariant) {
  Property(fn(instance) { material_variant.set_custom_physical_properties(instance, value) })
}
...
pub fn emissive_strength(value: Float) -> Property(MaterialVariant) {
  Property(fn(instance) { material_variant.set_emissive_strength(instance, value) })
}
...
pub fn material_pattern(value: MaterialPattern) -> Property(MaterialVariant) {
  Property(fn(instance) { material_variant.set_material_pattern(instance, value) })
}
...
pub fn javascript_type_anchor(_: Instance, _: SecurityCapabilities, _: MaterialPattern, _: Color3, _: PhysicalProperties, _: AlphaMode, _: MaterialVariant) -> Nil {
  Nil
}