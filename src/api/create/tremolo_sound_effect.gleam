...
import api/tremolo_sound_effect
import api/types.{type Instance, type SecurityCapabilities, type TremoloSoundEffect}
...
@target(luau)
pub fn node(properties: List(Property(TremoloSoundEffect)), children: List(Node)) -> Node {
...
@target(luau)
pub fn capabilities(value: SecurityCapabilities) -> Property(TremoloSoundEffect) {
  Property(fn(instance) { tremolo_sound_effect.set_capabilities(instance, value) })
}
...
pub fn javascript_type_anchor(_: Instance, _: SecurityCapabilities, _: TremoloSoundEffect) -> Nil {
  Nil
}