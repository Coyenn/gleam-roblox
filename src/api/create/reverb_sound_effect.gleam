...
import api/reverb_sound_effect
import api/types.{type Instance, type ReverbSoundEffect, type SecurityCapabilities}
...
@target(luau)
pub fn node(properties: List(Property(ReverbSoundEffect)), children: List(Node)) -> Node {
...
@target(luau)
pub fn capabilities(value: SecurityCapabilities) -> Property(ReverbSoundEffect) {
  Property(fn(instance) { reverb_sound_effect.set_capabilities(instance, value) })
}
...
pub fn javascript_type_anchor(_: Instance, _: SecurityCapabilities, _: ReverbSoundEffect) -> Nil {
  Nil
}