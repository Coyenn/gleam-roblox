...
import api/rotation_curve
import api/types.{type Instance, type RotationCurve, type SecurityCapabilities}
...
@target(luau)
pub fn node(properties: List(Property(RotationCurve)), children: List(Node)) -> Node {
...
pub fn javascript_type_anchor(_: Instance, _: SecurityCapabilities, _: RotationCurve) -> Nil {
  Nil
}