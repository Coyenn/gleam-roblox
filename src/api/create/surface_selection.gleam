...
import api/surface_selection
import api/types.{type BasePart, type Color3, type Instance, type NormalId, type SecurityCapabilities, type SurfaceSelection}
...
@target(luau)
pub fn node(properties: List(Property(SurfaceSelection)), children: List(Node)) -> Node {
...
pub fn target_surface(value: NormalId) -> Property(SurfaceSelection) {
  Property(fn(instance) { surface_selection.set_target_surface(instance, value) })
}
...
pub fn javascript_type_anchor(_: Instance, _: SecurityCapabilities, _: Color3, _: BasePart, _: NormalId, _: SurfaceSelection) -> Nil {
  Nil
}