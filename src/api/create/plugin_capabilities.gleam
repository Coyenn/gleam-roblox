...
import api/plugin_capabilities
import api/types.{type Instance, type PluginCapabilities, type SecurityCapabilities}
...
@target(luau)
pub fn node(properties: List(Property(PluginCapabilities)), children: List(Node)) -> Node {
...
@target(luau)
pub fn capabilities(value: SecurityCapabilities) -> Property(PluginCapabilities) {
  Property(fn(instance) { plugin_capabilities.set_capabilities(instance, value) })
}
...
pub fn javascript_type_anchor(_: Instance, _: SecurityCapabilities, _: PluginCapabilities) -> Nil {
  Nil
}